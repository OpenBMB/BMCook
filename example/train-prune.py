...
def main():
    ...
    # for eval
    #bmp.load(model, "/data/home/scv0540/zzy/gpt-j/bm-gpt-prune.pt")
    bmp.load(model, "results-prune/ckpt-10000.pt")
    ...
    BMPrune.compute_mask(model, m4n2_2d_greedy, checkpoint="gpt-j-mask.bin")
    ...
    for _ in range(3):
        for iteration, data in enumerate(Trainer.batch_iter(dataset, batch_size, bmp.rank(), bmp.world_size())):
            ...
            if iteration % 2000 == 0 and iteration != 0:
                bmp.save(model, "results-prune/ckpt-%d.pt" % iteration)
                # pass
