...
def pretrain(args, tokenizer, model, optimizer, lr_scheduler, dataset):
    ...
    if config.get('pruning')['is_pruning'] == False:
        train_mask, masks = False, None
    else:
        train_mask, masks = BMPrune.compute_mask(model, config)
    if train_mask:
    ...
    for iteration, data in enumerate(dataloader):
        ...
        if train_mask:
            ...
            masks = sprune_module.forward(training=True)
        ...
        if train_mask: 
            ...
            if iteration % 20 == 0:
                masks = sprune_module.forward(training=False)
                torch.save(sprune_module, 'results/sprune/300M_l0_module.pt')
                torch.save(masks, 'results/sprune/300M_head_mask.pt')
