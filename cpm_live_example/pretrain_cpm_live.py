...
def pretrain(args, tokenizer, model, optimizer, lr_scheduler, dataset):
    ...
    for iteration, data in enumerate(dataloader):
        ...
        if train_mask:
            sprune_module.zero_grad()
            sp_optimizer.zero_grad()
            lag_optimizer.zero_grad()
        
            bmt.print_rank('sprune_module forward pass and update the mask...')
            zs = sprune_module.forward(training=True)
        ...
        # ===========
        logits, _ = model(input_idx, input_length, input_context, input_position, input_segment, input_span, zs=zs)
        loss = loss_func(logits.view(-1, logits.size(-1)), targets.view(-1))
        global_loss = bmt.sum_loss(loss).item()
        mem_usage, tim_usage = add_mem_time('forward', mem_usage, tim_usage)
        if train_mask:
            lag_loss, expected_sparsity, _  = sprune_module.lagrangian_regularization()
            lag_loss.backward()
        ...
        if train_mask: 
            bmt.print_rank('expected_sparsity: ', expected_sparsity)
            if bmt.rank() == 0:
                writer.add_scalar("expected_sparsity", torch.mean(expected_sparsity), iteration)
            if iteration % 20 == 0:
                zs = sprune_module.forward(training=False)
                torch.save(sprune_module, 'results/sprune/300M_l0_module.pt')
                torch.save(zs, 'results/sprune/300M_head_mask.pt')
...
def main():
    ...
    dataset = CPMLive_Dataset(
        DistributedMMapIndexedDataset("data/merged/", "cpm_live_text_merge_context", bmt.rank(), bmt.world_size()),
