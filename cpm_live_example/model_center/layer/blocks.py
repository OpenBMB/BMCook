...
class TransformerBlock(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        1., 1., 0., 0., 0., 1., 1., 0., 1.]
        att_grad = False if self.att_z[index] == 0. else True
        ffn_grad = False if self.ffn_z[index] == 0. else True
    ...
    def forward(self,
        ...
        if not self.mask_att:
            if zs is not None:
                ...
                if 'self_att_layer_z' in zs:
                    att_layer_z = zs['self_att_layer_z'][att_index].clone().detach()
                    att_layer_z = att_layer_z.to(dtype=torch.half, device="cuda")
                if 'heads_z' in zs:
                    att_z = zs['heads_z'][att_index].clone().detach()
        ...
        if not self.mask_ffn:
            if zs is not None:
                ...
                if 'ffn_layer_z' in zs:
                    ffn_layer_z = zs['ffn_layer_z'][ffn_index].clone().detach()
                    ffn_layer_z = ffn_layer_z.to(dtype=torch.half, device="cuda")
                if 'dimff_z' in zs:
                    ffn_z = zs['dimff_z'][ffn_index].clone().detach()
