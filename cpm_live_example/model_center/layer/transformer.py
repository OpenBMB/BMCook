...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        if mask_modules is not None:
            ...
            for i, mask_module in enumerate(mask_modules):
                assert len(mask_module) == 2, "For encoder, each mask should be (mask_att, mask_ffn)"
                #if type(mask_module) is not tuple:
                #    mask_modules[i] = tuple(mask_module)
        else:
            ...
            self.layers = bmt.TransformerBlockList([
                bmt.CheckpointBlock(
                    TransformerBlock(
                        ...
                        index = index,
                        mask_att = mask_modules[index][0],
                        mask_ffn = mask_modules[index][1],
                    )
                )
...
!
        ...
        if not self.use_cache:
            hidden_states = self.layers(hidden_states, attention_mask, position_bias, None, None, None, zs)
